...
from utils import NASBenchwithLabel, graph2arch, save_arch, sample_random
...
for i in range(opt.n_perf_classes):
  print('Generate the architectures of label %d...\n' % i)
  z = FloatTensor(np.random.normal(0, 1, (N, 100)))
  gen_labels = LongTensor(np.ones(N)*i)
  ...
  for gen_conv_arch, gen_reduc_arch in zip(gen_conv_archs, gen_reduc_archs):
    conv_model_spec = utils.ModelSpec(matrix=gen_conv_arch[0], ops=gen_conv_arch[1])
    reduc_model_spec = utils.ModelSpec(matrix=gen_reduc_arch[0], ops=gen_reduc_arch[1])
    spec_list.append((conv_model_spec, reduc_model_spec))
  save_arch('archs/%d.txt' % i, spec_list, label_list, None, perf_list)
  torch.save(spec_list, 'archs_%d' % i)
