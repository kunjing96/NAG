...
for ratio in ratios:
    ...
    for epoch in range(opt.n_epochs):
        ...
        for i, (conv_edges, conv_nodes, conv_ns, reduc_edges, reduc_nodes, reduc_ns, perf_labels, _) in enumerate(nasbench_dataloader):
            ...
            real_labels = perf_labels.type(LongTensor).squeeze()
            ...
            gen_labels = LongTensor(np.random.randint(0, opt.n_perf_classes, conv_ns.shape[0]))

            # Generate a batch of images
            gen_conv_edges, gen_conv_nodes, gen_conv_ns, gen_reduc_edges, gen_reduc_nodes, gen_reduc_ns = generator(z, gen_labels)

            # Measure discriminator's ability to classify real from generated samples
            real_loss = adversarial_loss(discriminator(real_conv_edges, real_conv_nodes, real_conv_ns, real_reduc_edges, real_reduc_nodes, real_reduc_ns, real_labels), valid)
            fake_loss = adversarial_loss(discriminator(gen_conv_edges.detach(), gen_conv_nodes.detach(), gen_conv_ns.detach(), gen_reduc_edges.detach(), gen_reduc_nodes.detach(), gen_reduc_ns.detach(), gen_labels.detach()), fake)
            ...
            if i % opt.n_critic == 0:
                ...
                g_loss = adversarial_loss(discriminator(gen_conv_edges, gen_conv_nodes, gen_conv_ns, gen_reduc_edges, gen_reduc_nodes, gen_reduc_ns, gen_labels), valid)
        ...
        if (epoch+1) % 2 == 0:
            ...
            label_list = gen_labels.data.cpu().numpy().tolist()
            spec_list = []
            ...
            save_arch('archs/%d.txt' % batches_done, spec_list, label_list, None, None)
